...
def evaluate_model_performance():
    ...
    if len(steps) > 0 and len(val_losses) > 0:
        print(f"记录的数据点数量：steps={len(steps)}, val_losses={len(val_losses)}, train_losses={len(train_losses)}")
        ...
        plt.close()
        print(f"损失曲线已保存到 {out_dir}/training_loss.png")
    else:
        print("警告：没有足够的训练数据来绘制损失曲线")
